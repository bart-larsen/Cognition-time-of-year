...
source("~/Library/CloudStorage/Box-Box/Work/PennLINC/Code/Tools/GAM_Posterior_circular_min_max.R")
...
abcd.names<-fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/ABCD/ABCD_pull/abcd_tabulated_data-20230412/abcd_tbss01.txt',,na.strings = c("777"))
abcd<-fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/ABCD/ABCD_pull/abcd_tabulated_data-20230412/abcd_tbss01.txt',,na.strings = c("777"),skip = 2)
names(abcd)=names(abcd.names)

factorscores <- readRDS('~/Box/Work/UMN/Projects/Cognition_TOY/Data/ABCD/DEAP-data-download_ThompsonScores_ResFamIncome.rds')%>%
  filter(event_name=="baseline_year_1_arm_1")

exosomes <- fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/ABCD/abcd_longitudinal_factor_scores_7f_bifactor.csv')%>%
...
site <- readRDS("~/Box/Work/UMN/Projects/Cognition_TOY/Data/ABCD/DEAP-siteID.rds")%>% filter(event_name=="baseline_year_1_arm_1")
...
remove(abcd.cbcl.names,abcd.names,exosomes,factorscores,screen.time,screen.time.names,site,abcdy1,participants,abcd)
```
...
```{r data}
task_data_ar <- fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/PNC/go1_go2_go3_pnc_cnb_zscores_frar_20170202_wdates.csv')
task_data_nr <- fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/PNC/n9498_cnb_zscores_fr_20170202.csv')
task_data_factor <- fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/PNC/n9498_cnb_factor_scores_fr_20170202.csv')
demos <- fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/PNC/n9498_demographics_go1_20161212.csv')
pnc.exposome <- fread('~/Box/Work/UMN/Projects/Cognition_TOY/Data/PNC/PNC_exposome_bifactor_expandedENV.csv')%>%
  mutate(bblid=ID)

task_data <- left_join(task_data_factor,task_data_nr)%>%
  left_join(task_data_ar)%>%
  left_join(demos)%>%
  left_join(pnc.exposome)
# mutate variables 
pnc <- task_data%>%
...
```{r}
interview.dates <- fread('Data/ADHD1000/Visit_dates.csv',sep = ",",na.strings = "\n")
interview.dates <- read.csv('Data/ADHD1000/Visit_dates.csv',sep = ",",na.strings = " ") %>% mutate(across(contains("Date"),~as.Date(.x,format="%m/%d/%y")))
long.interview.dates <- interview.dates %>% 
  select(Mergeid,contains("Date"))%>%
  pivot_longer(cols = contains("Date"),names_to = "VisitLabel",values_to = "interview.date")%>%
  filter(!VisitLabel%in%c("V1_Date","Decision_Date"))%>%
  mutate(interview.year = year(interview.date),
         src_subject_id = factor(Mergeid))%>%
  mutate(VisitLabel = if_else(VisitLabel=="V2_Date","Y1",str_remove(VisitLabel, "_.*")))

# Prs stuff
prs.df <- fread('Data/ADHD1000/Oregon-ADHD-1000_imaging_demo_pheno_prs.csv',sep = ",")%>%
  mutate(src_subject_id = factor(MERGEID))
adhd.scores <- fread('Data/ADHD1000/OHSU_earliest_visit.csv',sep = ",")

# DTeam dx
dx.df <- fread('Data/ADHD1000/CleanY1-Y13Dteam.csv',sep = ",")
long.dx.df <- dx.df %>%
  select(Mergeid, matches("^Y([1-9]|1[0-3])_Adhd_Status$"))%>%
  pivot_longer(cols = contains("Adhd_Status"),names_to = "VisitLabel",values_to = "Status") %>%
  mutate(VisitLabel = str_remove(VisitLabel, "_Adhd_Status"))

# Join DTeam and interview dates
long.interview.dates <- long.interview.dates %>%
  left_join(long.dx.df)
...
adhd.cog.scores.dates <- adhd.cog.scores %>% 
  full_join(long.interview.dates%>%
              mutate(visit_year = as.numeric(str_extract(VisitLabel, "\\d+"))),by = c("visit_year", "src_subject_id")) %>%
  mutate(weekday = weekdays(interview.date),
         doy = yday(interview.date))%>%
  left_join(weekdaykey)

adhd.clin <- prs.df %>%
  left_join(adhd.scores)
adhd.df <- adhd.cog.scores.dates%>%
  left_join(adhd.clin,by = c("visit_year"="YEAR","src_subject_id"))%>%
  mutate(Y1_ADHD_STATUS = factor(Y1_ADHD_STATUS),
         sex = factor(sex),
...
```{r}
gusto.wasi <- fread('Data/GUSTO/GUSTO_cognitive_scores/GUSTO_WASI.csv')%>%
  separate(col = "ID",into = c("site","subject-id",sep = "-"))%>%
  select(!contains("Item"))%>%
  filter(`WASI usability`=="usable")
gusto.demographics <-fread('Data/GUSTO/GUSTO_cognitive_scores/GUSTO_demographics75.csv')%>%
...
gusto.cantabswm <- fread('Data/GUSTO/GUSTO_cognitive_scores/GUSTO_Y06_CANTAB SWM_20190104.csv')%>%
  separate(col = "SubjectID",into = c("site","subject-id",sep = "-"))
gusto.cantabdms <- fread('Data/GUSTO/GUSTO_cognitive_scores/GUSTO_Y06_CANTAB DMS_20190104.csv')%>%
  separate(col = "SubjectID",into = c("site","subject-id",sep = "-"))

gusto.data.cantabswm <- left_join(gusto.demographics,gusto.cantabswm,by=c("subject-id"))%>%
  mutate(age.in.days = SWM_Age_at_test_yr6*(365.25/12))%>% # convert age in months to days
  mutate(int_date = birthdate+age.in.days)
gusto.data.cantabdms <- left_join(gusto.demographics,gusto.cantabdms,by=c("subject-id"))%>%
  mutate(age.in.days = DMS_Age_at_test_yr6*(365.25/12))%>% # convert age in months to days
  mutate(int_date = birthdate+age.in.days)
...
gusto.data.cantabswm <- gusto.data.cantabswm%>%
  mutate(weekday = weekdays(int_date))%>%
  mutate(doy = yday(int_date))%>%
  mutate(monthnum=month(int_date))%>%
  mutate(sex=factor(sex,levels=c("M","F"),labels=c("M","F")),osex=ordered(sex))%>%
...
names(gusto.data.cantabswm)<-make.names(names(gusto.data.cantabswm))
setDT(gusto.data.cantabswm)

gusto.data.cantabdms <- gusto.data.cantabdms%>%
  mutate(weekday = weekdays(int_date))%>%
  mutate(doy = yday(int_date))%>%
  mutate(monthnum=month(int_date))%>%
  mutate(sex=factor(sex,levels=c("M","F"),labels=c("M","F")),osex=ordered(sex))%>%
  mutate(subid=factor(`subject-id`))%>%
  rename("DMS_A"=`DMS_A'_yr6`)%>%
  rename("DMS_B" = `DMS_B''_yr6`)%>%
  left_join(weekdaykey)

names(gusto.data.cantabdms)<-make.names(names(gusto.data.cantabdms))
setDT(gusto.data.cantabdms)
```
