...
weekdaykey<-data.frame(weekday=
                       ...
                       weekdaynum=
                         c(1,2,3,4,5,6,7))
dark_purples <- RColorBrewer::brewer.pal(n = 8, name = "Purples")[c(4,7)]
```
...
make.plot.df <- function(modobj,term,int.term=NULL,group.term=NULL,add.intercept=FALSE,xlabel=NULL,ylabel=NULL,plot.title = NULL,show.data=TRUE,newdata=NULL){
  ...
  if (is.null(newdata)) {
    ...
    pterms <- predict(modobj,type = "terms",se.fit = TRUE,newdata=newdata)
  }
  ...
  if (is.null(int.term)) {
    int.term="no interaction" #this just helps with column selection later
  }
  ...
  colnames(pterms.fit) <- gsub(x = colnames(pterms.fit),pattern = "ti\\(",replacement = "")%>%
    gsub(pattern = "\\)",replacement = "")
  colnames(pterms.sefit) <- gsub(x = colnames(pterms.sefit),pattern = "s\\(",replacement = "")%>%
    gsub(pattern = "\\)",replacement = "")

  pterms.df <- data.frame(pterms.fit)%>%select(any_of(terms),contains(paste0(term,".",int.term)),contains(paste0(int.term,".",term)))%>%
    mutate(fit=rowSums(across(where(is.numeric))))%>%
    select(fit)%>%
    # rename(fit:=!!term)%>%
    # cbind(data.frame(pterms.sefit)%>%select(matches(term))%>%rename(se.fit:=!!term))%>%
    cbind(data.frame(pterms.sefit)%>%select(any_of(terms),contains(paste0(term,".",int.term)))%>%mutate(se.fit=rowSums(across(where(is.numeric)))))%>%
  ...
  if (is.null(newdata)) {
    plot.df <- cbind(pterms.df,df%>%select(matches(terms))%>%rename_all(.funs = function(x) paste0(x,".raw")))
  } else {
    plot.df <- cbind(pterms.df,newdata%>%select(matches(terms))%>%rename_all(.funs = function(x) paste0(x,".raw")))
  }
...
abcd.names<-fread('Data/ABCD/ABCD_pull/abcd_tabulated_data-20230412/abcd_tbss01.txt',,na.strings = c("777"))
abcd<-fread('Data/ABCD/ABCD_pull/abcd_tabulated_data-20230412/abcd_tbss01.txt',,na.strings = c("777"),skip = 2)
names(abcd)=names(abcd.names)

factorscores <- readRDS('Data/ABCD/DEAP-data-download_ThompsonScores_ResFamIncome.rds')%>%
  filter(event_name=="baseline_year_1_arm_1")

exosomes <- fread('Data/ABCD/abcd_longitudinal_factor_scores_7f_bifactor.csv')%>%
  mutate(eventname=Timepoint,src_subject_id=ID)

abcdy1 <- abcd %>% filter(eventname=="baseline_year_1_arm_1")%>%
  left_join(factorscores)%>%
  left_join(exosomes)
#Get Day of Week
weekdaykey<-data.frame(weekday=
                         c("Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday","Monday"),
...
# Get site (this is now included with participants.tsv)
site <- readRDS("Data/ABCD/DEAP-siteID.rds")%>% filter(event_name=="baseline_year_1_arm_1")

abcd.df <- abcd.df %>% left_join(site)%>%
  mutate(rel_family_id=factor(rel_family_id),
         src_subject_id=factor(src_subject_id),
         eventname = factor(eventname))

# Temperature
temp.data <- fread("Data/TemperatureAnalysis/3920706.tempexcel.csv")%>%
  mutate(Zipcode = `Zip Code`)
temp.site <- fread("data/TemperatureAnalysis/ABCD_Sites.csv")
temp.df <- temp.data %>% left_join(temp.site)%>%
  mutate(int_date=as.Date(DATE,format = "%m/%d/%Y"),
         site = `Site ID`)
...
remove(abcd.names,exosomes,factorscores,site,abcdy1,participants,abcd)
...
#### Monthly variation
Figure S1A. Cognition varies by month of the year.  
...
plot.data.abcd <- model_output %>%
  mutate(predictions = map(model, ~ ggemmeans(.x, terms = "monthnum"))) %>%
  unnest(predictions) %>%
  select(-model)
...
month.plot <- ggplot(plot.data.abcd,
  ...
  ylab("Performance") +
  ggtitle("Cognition by month") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.x = element_blank())

month.plot

cowplot::save_plot('Figs/SupplementaryFigure1_month.svg',month.plot,base_height = 4,base_width = 5)
```

#### End of summer (end of summer break) vs spring (before summer break) 
Figure S1B. Cognition is higher in spring (before summer break) than in late summer (after summer break). Response to reviewer comment.
```{r}
# Create the time_group variable separating end of summer visits from spring visits (pre-summer)
# End of Summer: DOY 238-268. Spring: DOY 32-151.
longdf <- abcd.df %>%
  mutate(
    time_group = case_when(
      doy >= 238 & doy <= 268 ~ "End of Summer",
      doy >= 32 & doy <= 151 ~ "Spring",
      TRUE ~ "Other"
    )
  ) %>%
  pivot_longer(
    cols = c(neurocog_pc1.bl, neurocog_pc2.bl, neurocog_pc3.bl),
    names_to = "Factor",
    values_to = "score"
  ) %>%
  mutate(
  ...
  filter(time_group %in% c("Spring", "End of Summer")) %>%
  # Set Spring as the reference level
  mutate(
    time_group = relevel(factor(time_group), ref = "Spring")
  )


# Create equal sized groups
set.seed(32)
end_of_summer_count <- longdf %>%
  filter(time_group %in% c("Spring", "End of Summer")) %>%
  group_by(Factor,time_group) %>%
  count() %>%
  filter(time_group=="End of Summer")# 896 at end of summer

balanced_data <- longdf %>%
  filter(time_group %in% c("Spring", "End of Summer")) %>%
  group_by(Factor,time_group) %>%
  slice_sample(n = end_of_summer_count$n[[1]], replace = FALSE) %>%
  ungroup()%>%
  mutate(time_group = relevel(factor(time_group), ref = "Spring"))

# Run the Final Dummy-Coded Analysis on the Balanced Data
# Fit a separate bam model for each cognitive factor
model_output <- balanced_data %>%
  group_by(Factor) %>%
  summarise(
    model = list(
      bam(
...
tidy_output <- model_output %>%
    mutate(stats = map(model, function(m) {
      tidy_df <- tidy(m, parametric = TRUE) %>%
        filter(term == "time_groupEnd of Summer") %>%
        select(term, estimate, std.error, p.value)
      
      eta_df <- eta_squared(m) %>%
        filter(Parameter == "time_group") %>%
        select(Eta2_partial)
      
      bind_cols(tidy_df, eta_df)
    })) %>%
    unnest(stats) %>%
    select(Factor, term, estimate, std.error, p.value, Eta2_partial)

tidy_output %>% 
    kableExtra::kbl(escape = F) %>%
    kableExtra::kable_classic(html_font = "Cambria")

# Plot and show stats marking signficance
# Create significance labels
sig_labels <- tidy_output %>%
  mutate(
    sig_label = case_when(
      p.value < 0.001 ~ "***",
      p.value < 0.01 ~ "**",
...
# Generate predictions and join with significance data
plot.data <- model_output %>%
  mutate(plot.df = map(model, ~ ggeffects::ggemmeans(.x, terms = "time_group"))) %>%
  unnest(plot.df) %>%
  select(-model) %>%
  left_join(sig_labels, by = "Factor")

# Calculate y-position for significance brackets (per Factor)
signif_data <- plot.data %>% 
  group_by(Factor) %>% 
  summarize(
    y_pos = max(conf.high) + .02,
    sig_label = first(sig_label)
  )

# Create plot
spring_summer_plot <- ggplot(plot.data, aes(x = x, y = predicted)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2) +
  geom_signif(
    data = signif_data,
    aes(xmin = 1, xmax = 2, annotations = sig_label, y_position = y_pos),
    manual = TRUE,
    tip_length = 0.02
  ) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.15))) +
  labs(y = "Performance", title = "Spring and end of summer") +
  theme_classic() +
...
# Figure S1
Figure.S1 <- month.plot + spring_summer_plot + plot_layout(ncol = 2, widths = c(1, .6))
Figure.S1
cowplot::save_plot('Figs/SupplementaryFigure1.svg',Figure.S1,base_height = 4,base_width = 6)
```
...
abcd_factors=model_smooths%>%
  mutate(upper=.estimate+1.96*.se,lower=.estimate-1.96*.se)%>%
...
This code just repeats what we did for the whole sample for two demographically match splits (the ARMS).  
...
#### Plot abcd factor results for each ARM split

Do the plotting for each ARM split.  
...
lines.split #display the line plot
radial.split #display the radial plot
Supplementary.Figure.2 <- lines.split + radial.split + plot_layout(nrow=1,heights = c(.8,1),widths = c(.66,1))
cowplot::save_plot('Figs/SupplementaryFigure2_ABCD.splits.svg',Supplementary.Figure.2,base_height = 4,base_width = 12)

```

#### Random splits
Re-fit the models for n=20 random splits of the data. 

```{r,echo=FALSE,cache=TRUE}

# Set number of random splits
n_splits <- 20

set.seed(23)

# Function to process one random split
process_split <- function(split_num, data) {
  ...
  # Prepare long format
  longdf <- sampled_data %>%
    pivot_longer(cols = c("neurocog_pc1.bl", "neurocog_pc2.bl", "neurocog_pc3.bl"), names_to = "Factor", values_to = "score") %>%
    mutate(Factor = factor(Factor, levels = c("neurocog_pc1.bl", "neurocog_pc2.bl", "neurocog_pc3.bl"), 
                          labels = c("General Cognition", "Executive Function", "Learning/Memory")))
  ...
  model_output <- longdf %>%
    ...
    summarise(model = list(bam(
      ...
      method = "REML")),
      .groups = "drop") %>%
    mutate(split_id = split_num)
  
  return(model_output)
}

# Filter data once
filtered_data <- abcd.df 

# Run for all splits
model_output <- map_dfr(1:n_splits, ~process_split(.x, filtered_data))

# Rest of your analysis code stays the same
tidy_model <- model_output %>%
  mutate(tidy = map(model, function(x) {
    tidydf = broom::tidy(x)
    tidydf$N = length(x$residuals)
...
tidy_model %>% 
  kableExtra::kbl(escape = F, caption = "Model output for random splits and factors") %>% 
  kableExtra::kable_classic(html_font = "Cambria")

plot.df.abcd.splits <- model_output %>%
  mutate(plot.df = map(model,
                       .f = ~ make.plot.df(.x, term = "doy", add.intercept = FALSE))) %>%
  unnest(plot.df) %>%
  select(-model)

# get the minima
abcd.factors.mins.splits.random <- model_output %>% 
  mutate(minmax = map(.x = model, .f = ~ gam.posterior.smooths.circular(.x, return_draws = FALSE, smooth_var = "doy"))) %>% 
  unnest(minmax) %>%
  select(split_id, Factor, minmax) %>%
  unnest(minmax) %>%
  filter(is.na(max.y)) %>%
  mutate(across(.cols = where(is.numeric), .fns = ~ lubridate::parse_date_time(as.character(floor(.)), orders = "j")))
...
#### Plot the random splits
Figure 1A bottom.  

```{r}
# Build the line plot depicting the ToY effect across random splits
lines.split <- ggplot(plot.df.abcd.splits, aes(x = doy.raw, y = fit,group = split_id, ymin = lwr, ymax = upr,color = Factor, fill=Factor)) +
  # geom_ribbon(alpha = .1, color = NA) +
  geom_line() +
  scale_color_brewer(type = "seq", palette = "Reds", direction = -1) +
  scale_fill_brewer(type = "seq", palette = "Reds", direction = -1) +
...
## Build points dataframe - just the minima points, no rectangles needed
points.tibble <- abcd.factors.mins.splits.random %>%
  mutate_if(is.POSIXct, as.Date) %>%  # Convert all date-time columns to date for easier comparison
  mutate_if(is.Date, as.POSIXct)

# Plot all points together
radial.split <- ggplot(points.tibble, aes(y = as.numeric(Factor),
                                           x = min.y,
                                           fill = Factor,
                                           color = Factor)) +
  geom_point(alpha = 0.5, size = 2) +  # Add some transparency since there will be many points
  scale_color_brewer(type = "seq", palette = "Reds", direction = -1) +
  scale_fill_brewer(type = "seq", palette = "Reds", direction = -1) +
  scale_x_datetime(date_labels = "%B",
                   limits = lubridate::parse_date_time(c("1", "365"), orders = "j"),
  ...
  theme(axis.text.x = element_text(hjust = 1, vjust = -1),
        ...
        plot.title = element_text(face = "plain")) +
  coord_polar(start = -pi/365, clip = "off") +
  ggtitle("Day of Minimum Performance Across Random Splits")

cowplot::save_plot('Figs/ABCD.random.lines.svg',lines.split)
cowplot::save_plot('Figs/ABCD.random.radial.svg',radial.split)
Figure.1A.Bottom <- lines.split + radial.split 
Figure.1A.Bottom
cowplot::save_plot('Figs/Figure1A.bottom.svg',Figure.1A.Bottom,base_height = 5,base_width = 10)

```


## Demographics
Examine how the effect of calendar day varies by demographic variables.  

### Sex
```{r, sex, fig.height=7, fig.width=6, cache=TRUE}
...
custom_breaks <- c(ymd_hms("2025-01-01 00:00:00"), 
                   seq(from = ymd_hms("2025-04-01 00:00:00"), 
                       to = ymd_hms("2025-12-01 00:00:00"), 
                       by = "3 months"),
                   ymd_hms("2025-12-31 00:00:00"))
...
rplot2<-ggplot(gmod.sex$model,
  ...
  theme(legend.position="none",legend.title = element_blank(),axis.title=element_blank(),axis.text.x = element_text(angle = 45,hjust=1,vjust=1),axis.text.y=element_blank(),axis.ticks.y = element_blank())
...
### Education
```{r, educ, fig.height=7,fig.width=4, cache=TRUE}
...
abcd.df$household.income <- relevel(abcd.df$household.income,ref = "[>=50K & <100K]")
...
inc.plot.df <- make.plot.df(gmod.inc,term = "doy", int.term = "household.income") %>% 
  mutate(date_labels = lubridate::parse_date_time(as.character(doy.raw),orders="j"))%>%
  mutate(fac.inc = relevel(factor(household.income.raw),ref="[<50K]"))
...
```{r magnitude of effect,cache=TRUE}
...
d_model %>%
  ...
  mutate(message = sprintf(
    "Kids lose %1.2f days of development in %s for each day of summer",
    # Calculate the proportion of yearly age effect for summer months
    # abs(minslope[smooth %in% c("s(doy)")] / maxslope[smooth %in% c("s(age)")]),
    abs(minslope[.smooth == "s(doy)"] / maxslope[.smooth == "s(age)"]),
    Factor
  )) %>%
  distinct(message) %>%
...
model_output <- longdf %>%
  ...
  summarise(
    model = list(bam(
      score ~ 
        s(age, k = 4) + 
        s(General_SES) +                      # Adjusted income variable included in model
        s(weekdaynum, bs = "cc", k = 7) + 
        s(doy, bs = "cc", k = 52) + 
        s(doy, abcd_site, bs = "fs", xt = list(bs = "cc", k = 26)) + 
        sex,
...
ggplot(abcd_factors2, aes(x = doy, y = .estimate, ymin = lower, ymax = upper, color = Factor, fill = Factor)) +
...
abcd.site.factors.mins2 <- abcd_factors2 %>% group_by(Factor,abcd_site)%>%
  summarize(min.day = doy[which.min(.estimate)])%>%
...
Just repeating for every task. Now included in figure 1B.  

```{r, eval=TRUE, cache=TRUE}
longdf <- abcd.df %>% 
  pivot_longer(cols = contains("uncorrected"), names_to = "cogtest", values_to = "score") %>%
  mutate(
    matched_group = factor(matched_group),
    cogtest = str_remove(string = cogtest, pattern = "_uncorrected"),
    cogtest = str_replace(string = cogtest, pattern = "nihtbx_", replacement = ""),
    cogtest = factor(cogtest, 
                    levels = c("cardsort", "cryst", "flanker", "fluidcomp", 
                              "list", "pattern", "picture", "picvocab", 
                              "reading", "totalcomp"),
                    labels = c("Card Sort", 
...
abcd.tasks.lines <- ggplot(abcd_tasks,
  aes(x=doy,y=.estimate,color=cogtest,fill=cogtest,ymin=lower,ymax=upper))+
  geom_ribbon(alpha=.1,color=NA)+
  geom_line()+
  scale_x_continuous(breaks = seq(1, 365, by = 120), labels = function(doy) sapply(doy, doy_to_month)) +
  ...
  ggtitle("ABCD NIH Toolbox")+
  ylab("Day-of-year effect")
  # facet_wrap(~matched_group,labeller = labeller(matched_group = 
    # c("1" = "Split 1",
    #   "2" = "Split 2")))
...
pnc <- pnc%>%
  ...
  mutate(race.factor=factor(race, levels = 1:6,labels=levels(abcd.df$race.6level)))%>%
  left_join(weekdaykey)%>%
...
lines.nine <- ggplot(plot.data.pnc.young,aes(x = doy.raw, y = fit, ymin = lwr, ymax = upr, color = Factor, fill = Factor)) +
  ...
  scale_x_continuous(breaks = seq(1, 365, by = 120), labels = function(doy) sapply(doy, doy_to_month)) +
  theme(axis.text.x = element_text(ange=60,hjust=1),axis.title.x = element_blank(),legend.title = element_blank(), legend.position = "top")+
  ylab("Performance")+
...
pnc.lines.old.young <- ggplot(plot.data.pnc.all %>% na.omit(),
  ...
  scale_x_continuous(breaks = seq(1, 365, by = 120), labels = function(doy) sapply(doy, doy_to_month)) +
  theme(axis.text.x = element_text(ange=60,hjust=1),axis.title.x = element_blank())+
  ylab("Performance")+
...
adhd.plot.data <- model_output.by %>%
  mutate(plot.df = map(.x = model,
                       .f = ~ make.plot.df(.x, term = "doy", int.term = c("interview_age", "oADHD_Status"), newdata = newdata) %>%
                         mutate(date_labels = lubridate::parse_date_time(as.character(doy.raw), orders = "j")))) %>%
  unnest(plot.df) %>%
  select(-model)

# Create plot comparing ADHD vs neurotypical performance at age 9.9
supp.adhd.typical.9y <- ggplot(adhd.plot.data %>% mutate(oADHD_Status = factor(oADHD_Status.raw, levels = c("ADHD", "Control"), labels = c("ADHD", "Neurotypical"))),
       aes(x = doy.raw, y = fit, ymin = lwr, ymax = upr, color = oADHD_Status, fill = oADHD_Status)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.x =  element_blank(), legend.position = "top", legend.title = element_blank()) +
  facet_wrap(~cog.scale, scales = "free_y", nrow = 3) +  # Create facets for each cognitive scale
  ylab("Performance (z-score)") +  # Y-axis label
  ggtitle("Performance at age 9.9y")  # Title for the plot
...
newdata =  expand_grid(interview_age=c(median(as.numeric(abcd.df$interview_age))), #,
                       doy=1:365,
                       oADHD_Status = c("ADHD"), #,"ADHD"
                       sex="M",
                       visit_year = levels(adhd.df$visit_year)[1],
                       weekdaynum=2,
                       src_subject_id = levels(adhd.df$src_subject_id)[1])

adhd.factors.mins <- model_output.by %>% 
  mutate(minmax = map(.x=model,.f=~gam.posterior.smooths.circular(.x,
                                                                  return_draws = FALSE,
                                                                  smooth_var="doy",
                                                                  newdata = newdata,
                                                                  cov_list=c("interview_age","oADHD_Status")))) %>%
  unnest(minmax)%>%
...
newdata =  expand_grid(interview_age=c(median(as.numeric(abcd.df$interview_age))), #,
                       doy=1:365,
                       oADHD_Status = c("Control"), #,"ADHD"
                       sex="M",
                       visit_year = levels(adhd.df$visit_year)[1],
                       weekdaynum=2,
                       src_subject_id = levels(adhd.df$src_subject_id)[1])

control.factors.mins <- model_output.by %>% 
  mutate(minmax = map(.x=model,.f=~gam.posterior.smooths.circular(.x,
                                                                  return_draws = FALSE,
                                                                  smooth_var="doy",
                                                                  newdata = newdata,
...
pcplot.adhd <- ggplot(plot.data.adhd%>%filter(interview_age.raw==119),aes(x=doy.raw,y=fit,color=ADHD_Status.raw ,fill=ADHD_Status.raw ,ymin=lwr,ymax=upr,group=ADHD_Status.raw ))+
  ...
  scale_color_manual(aesthetics = c("fill","color"),
                     ...
                     name = NULL)+
  theme(axis.text.x=element_text(angle = 45,hjust = 1),axis.title.x =  element_blank(),legend.position = "top",legend.title.position = "top")+
  ylab("PC1")
pcplot.adhd
...
pcplot.combo <- ggplot(plot.data.adhd,aes(x=doy.raw,
  ...
  theme(axis.text.x=element_text(angle = 45,hjust = 1),axis.title.x =  element_blank(),legend.position = "top",legend.title.position = "top",plot.title = element_text(face = "plain"))+theme(strip.background = element_blank(),legend.position = "top")
...
gusto.wasi <- fread('Data/GUSTO/GUSTO_WASI.csv')%>%
  separate(col = "ID",into = c("site","subject-id",sep = "-"))%>%
  select(!contains("Item"))%>%
  filter(`WASI usability`=="usable")
gusto.demographics <-fread('Data/GUSTO/GUSTO_demographics75.csv')%>%
  mutate(across(.cols="subject-id",.fns=~str_remove(.x,pattern="GUSTO")))%>%
  mutate(birthdate = as.Date(as.character(`date of birth`),format="%Y%m%d"))


gusto.data <- left_join(gusto.wasi,gusto.demographics,by=c("subject-id"))
# mutate variables 
gusto.data <- gusto.data%>%
  mutate(int_date=as.Date(`Date of administration`,format = "%Y-%m-%d"))%>%
  mutate(weekday = weekdays(int_date))%>%
  mutate(doy = yday(int_date))%>%
  mutate(monthnum=month(int_date))%>%
...
rect.tibble <- factor.mins %>%
  ...
  filter(upper>lower)%>%
  mutate_if(is.Date, as.POSIXct)%>%
  mutate(study = str_replace(study, pattern = "ABCD Total", replacement = "ABCD"),
         study = factor(study))%>%
  filter(!str_detect(study, "ABCD Split"))%>%
...
# Covary for every single exposome factor
gmod.exposome.covses <- bam(neurocog_pc1.bl~
                       s(weekdaynum, bs = "cc", k = 7)+
                       s(doy,bs = "cc", k = 52)+
                       s(General_SES,k=4)+
                         s(School,k=4)+s(Family_Values,k=4)+s(Family_Turmoil,k=4)+s(Dense_Urban_Poverty,k=4)+s(Extracurriculars,k=4)+s(Screen_Time,k=4)+
                       s(age)+
                       s(doy, abcd_site, bs = "fs", xt = list(bs="cc",k=26)) +
                       osex,
                     data = abcd.df,REML = TRUE)
...
reduced.model <- bam(neurocog_pc1.bl~
                       s(weekdaynum, bs = "cc", k = 7)+
                       s(General_SES)+s(School,k=4)+s(Family_Values,k=4)+s(Family_Turmoil,k=4)+s(Dense_Urban_Poverty,k=4)+s(Extracurriculars,k=4)+s(Screen_Time,k=4)+
                       s(age)+
                       s(abcd_site, bs = "re") +
                       osex,
                     data = abcd.df,REML = TRUE)
...
#### Does DOY survive when propensity matching on exposome scores?
```{r exposome doy propensity}
set.seed(42)

# Create a new binary variable for assessments in spring vs end of summer (pre-post summer break)
# End of summer will be defined as the confidence interval for day of minimum performance: August 27 and September 25
# August 27 is DOY 239. September 25 is DOY 268.

abcd.df <- abcd.df %>%
  mutate(
    end_of_summer_vs_spring = case_when(
      doy >= 239 & doy <= 268 ~ 1, # End of summer group
      doy >= 32 & doy <= 151 ~ 0, # Spring group
      TRUE ~ NA_real_           # All other DOYs are set to NA
    )
  )

# Only keep the data for the two groups of interest
abcd.df.sub <- abcd.df %>%
  filter(!is.na(end_of_summer_vs_spring), !is.na(General_SES), !is.na(neurocog_pc1.bl))%>%
  mutate(end_of_summer_vs_spring = factor(end_of_summer_vs_spring, labels = c("Spring", "End of Summer")))

# Propensity Score Estimation across all exposome factors
ps.model <- glm(end_of_summer_vs_spring ~ General_SES + age + osex +
  School + Family_Values + Family_Turmoil + Dense_Urban_Poverty +
  Extracurriculars + Screen_Time, data = abcd.df.sub, family = binomial)
abcd.df.sub$pscore <- predict(ps.model, type = "response")
summary(ps.model)

# Use the propensity score to match individuals from the two groups
matchit.out <- MatchIt::matchit(end_of_summer_vs_spring ~ pscore, data = abcd.df.sub, method = "nearest", ratio = 1)
matched.data <- MatchIt::match.data(matchit.out)

# Now, the main model can directly compare the two groups, adjusting for
# any residual imbalance.
gmod.exposome.matched <- bam(neurocog_pc1.bl ~
  end_of_summer_vs_spring +
  s(age) +
  osex +
  s(abcd_site, bs = "re") +
  s(weekdaynum, bs = "cc", k = 7) +
    General_SES + School + Family_Values + Family_Turmoil + Dense_Urban_Poverty +
  Extracurriculars + Screen_Time,
  data = matched.data,
  REML = TRUE)

summary(gmod.exposome.matched)
eta_squared(gmod.exposome.matched)
effectsize::cohens_f_squared(gmod.exposome.matched)
sjPlot::plot_model(gmod.exposome.matched, type = "pred", terms = c("end_of_summer_vs_spring"),axis.labels = c("Spring", "End of Summer"),colors = "black") +
  xlab("Assessment Time") +
  ylab("General Cognition") +
  ggtitle("ABCD: End of Summer vs Spring (Propensity Matched on Exposome)") + theme_classic()

propensity_effect.plot <- ggeffects::ggemmeans(gmod.exposome.matched, terms = c("end_of_summer_vs_spring")) %>%
  plot() +
  xlab("Assessment Time") +
  ylab("General Cognition") +
  ggtitle("ABCD: End of Summer vs Spring\n (Propensity Matched on Exposome)") + theme_classic()
propensity_effect.plot
cowplot::save_plot("Figs/SES.ABCD.propensity.svg",propensity_effect.plot,base_width = 4, base_height = 4)

```

#### Test general exposome with all cognitive factors and FDR
```{r exposome all cog factors}
...
```{r, pnc.exposome all cog factors}
long.exposome <- pnc %>% 
...
```{r winter holiday analysis}
# Define the new groups for the Christmas holiday
# December is DOY 335-365. January is DOY 1-31.
abcd.df <- abcd.df %>%
  mutate(
    christmas_vs_january = case_when(
      doy >= 330& doy <=354 ~ "December",   # December group
      doy <= 25 ~ "January",    # January group
      TRUE ~ "Other"
    )
  )

# Subset the data for the two groups of interest
abcd.df.sub_xmas <- abcd.df %>%
  filter(christmas_vs_january %in% c("December", "January")) %>%
  mutate(christmas_vs_january = as.factor(christmas_vs_january))

# Run the analysis on this new subset
gmod.xmas <- bam(neurocog_pc1.bl ~
  christmas_vs_january +
  s(age) +
  osex +
    s(General_SES,k=4)+
  s(abcd_site, bs = "re") +
  s(weekdaynum, bs = "cc", k = 7),
  data = abcd.df.sub_xmas,
  REML = TRUE)
